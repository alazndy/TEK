...
interface UseHistoryReturn<T> {
  ...
  saveCheckpoint: (newState: T) => void;
  undo: () => void;
  redo: () => void;
  canUndo: boolean;
  canRedo: boolean;
  ...
  pastStates: T[];
  snapshots: { id: string; name: string; date: Date; state: T }[];
...
export function useHistory<T>(options: UseHistoryOptions<T>): UseHistoryReturn<T> {
  ...
  const [snapshots, setSnapshots] = useState<{ id: string; name: string; date: Date; state: T }[]>([]);

  const saveCheckpoint = useCallback((newState: T) => {
      setState(newState);
  }, [setState]);

  const createSnapshot = useCallback((name: string, state: T) => {
      setSnapshots(prev => [...prev, { id: crypto.randomUUID(), name, date: new Date(), state }]);
  }, []);
  ...
  return {
    ...
    setState,
    saveCheckpoint, // Alias for manual checkpoint
    undo,
    redo,
