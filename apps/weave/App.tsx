...
export default function App() {
  ...
  if (isLoading) {
    return <div className="flex items-center justify-center h-screen bg-zinc-900 text-white">Yükleniyor...</div>;
  }
  ...
  const initialPages = [createNewPage(1)];
  const initialActivePageId = initialPages[0].id;
  ...
  const { 
      ...
      initialState: { pages: initialPages, activePageId: initialActivePageId, templates: initialTemplates }
  });

  const { pages, activePageId } = historyState;

  // Sync templates from history to library manager on Undo/Redo
  useEffect(() => {
      if (historyState.templates && historyState.templates !== templates) {
          setTemplates(historyState.templates);
      }
  }, [historyState.templates, setTemplates]); 

  // Setters wrappers - ensure current templates are captured in history
  const setPages = (newPages: Page[] | ((prev: Page[]) => Page[])) => {
      setHistoryState(prev => ({
          ...prev,
          templates: templates, // Capture current templates
          pages: typeof newPages === 'function' ? newPages(prev.pages) : newPages
      }));
  };

  const setActivePageId = (newId: string | ((prev: string) => string)) => {
      setHistoryState(prev => ({
          ...prev,
          templates: templates, // Capture current templates
          activePageId: typeof newId === 'function' ? newId(prev.activePageId) : newId
      }));
  };

  // We don't wrap setTemplates because useLibraryManager owns it.
  // Instead, manual checkpoints (performCheckpoint) or interactions that call setTemplates 
  // should ideally also update history if we want granular history.
  // But for now, major actions call performCheckpoint.
  ...
  }, [setHistoryState, pages, activePageId, templates]);
    ...
    <div className="flex h-screen w-full bg-ink text-alabaster overflow-hidden font-sans select-none transition-colors duration-300 relative">
         ...
         <div className="flex-1 flex overflow-hidden relative">
            <LeftSidebar
                ...
                onOpenProjectBoard={() => setIsProjectBoardOpen(true)}
      ...
      <ModalManager
        ...
        isProjectBoardOpen={isProjectBoardOpen}
        setIsProjectBoardOpen={setIsProjectBoardOpen}
